...
//        print_r($this->city);
        $query = "SELECT `id`, `title`, `ownerId` FROM `pages` WHERE `city_id`='".$this->cityAdmin."'";
    ...
    protected function listPagesTree($owner)
    {
        ...
        $query = "SELECT `id`, `title`, `position` FROM `pages` WHERE `ownerId` =" . (int)$owner . " && `lang` = '" . $_SESSION['post'] . "' && `city_id` = '".$this->cityAdmin."' ORDER BY `position`";
        ...
        if ($mysql->num_rows() > 0) {
            $rows = $mysql->getList();
            foreach ($rows as $row) {
                $query = "SELECT `id`, `title` FROM `pages` WHERE `ownerId` = " . (int)$row['id'] . " && `lang` = '" . $_SESSION['post'] . "' && `city_id` = '".$this->cityAdmin."' ORDER BY `position`";
                $mysql->query($query);
    ...
    private function getCityIdByName($name){
        global $sql;
        $sql->query("SELECT `id` FROM `city` WHERE `name`='" . $name . "' LIMIT 1", true);
